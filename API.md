...
to the per-element arithmetic and ~1.2x faster on the A76 for feature-map-shaped inputs. This is **wired**: the gate accepts int8/uint8 `CONV_2D`,
...
- **UINT8 DIRECT** = the same `rocket_conv2d_int8` runtime fed RE-CENTERED bytes
  ...
  symmetric-only trick fails, and the box-sum is what makes it exact. **All 66 MobileDet CONV_2D run
...
the BS_MUL per-OC multiply path; uint8 DW native would match Teflon rather than CPU TFLite, a perf play with
...
round-trip at that boundary. Win: a MobileNetV2 block warm `12.66->9.34 ms` (1.36x), SSD head
...
NPU cores, default 4. It sizes **both** the 1×1 matmul fan-out **and** the native
...
- **Throughput comes from a process pool, not a faster single stream.** A single inference is host
  ...
  distinct A76 core via the `ROCKET_CPU_AFFINITY` env var: 3.20 -> 9.55 detection_fps at P=1->4 (2.98x,
  live Frigate). `nthreads` (default 4) fans a single conv across the 3 NPU cores; the pool fans across
  streams, and the pool is the larger lever.
...
- **Host requant** is the largest remaining host cost, and is fully NEON-vectorized.
  ...
  per step with no scatter. Bit-identical to the scalar `lrintf` epilogue, since every intermediate
  fits int32 and `vcvtnq_s32_f32` is its round-nearest-ties-even, and worth **+5.5 % warm MobileDet /
  ...
  microbench **2.6x (3×3) / 3.3x (5×5) / 4.3x (7×7) over the per-window NEON** (~10-16x over scalar);
  ...
  **NCHW direct-conv requant** (`rocket_out_nchw_to_nhwc_q_per_axis_band` signed + `_u8_band` uint8)
  ...
  `dst[pix·OC+oc]` is strided, so the per-pixel compute vectorizes over 4 contiguous source pixels and
  ...
  **1.6-3.4x**, but whole-model impact is sub-1 % (most MobileDet convs route to the already-vectorized
...
- **Throughput pool for multi-camera.** Warm single-stream ~336 ms is not video-rate;
  ...
  (`tools/pool_throughput.py`, a 1×1-conv submit-bound unit, 600 MHz): **P=1->4 = 1.00x / 2.17x /
  3.11x / 3.56x** aggregate inferences/s, end to end through the delegate, matching the driver
  ...
  that every process's output hash equals the single-process reference (measured 3.51-3.61x at
  ...
  aggregate `detection_fps` **P=1->4 = 3.20 / 6.00 / 8.00 / 9.55 = 1.00x / 1.88x / 2.50x / 2.98x**
  ...
  4x. Under that contended pool the
